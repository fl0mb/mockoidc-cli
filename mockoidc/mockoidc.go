...
func (m *MockOIDC) Start(ln net.Listener, cfg *tls.Config) error {
	...
	handler.Handle("/", m.chainMiddleware(m.Default))
	handler.Handle(IssuerBase+AuthorizationEndpoint, m.chainMiddleware(m.Authorize))
	handler.Handle(IssuerBase+TokenEndpoint, m.chainMiddleware(m.Token))
	handler.Handle(IssuerBase+UserinfoEndpoint, m.chainMiddleware(m.Userinfo))
	handler.Handle(IssuerBase+JWKSEndpoint, m.chainMiddleware(m.JWKS))
	handler.Handle(IssuerBase+DiscoveryEndpoint, m.chainMiddleware(m.Discovery))
...
func (m *MockOIDC) Addr() string {
	...
	port := strings.Split(m.Server.Addr, ":")[1]
	ip := strings.Split(m.Server.Addr, ":")[0]
	return fmt.Sprintf("%s://%s:%s", proto, ip, port)
}

// Issuer returns the OIDC Issuer that will be in `iss` token claims
func (m *MockOIDC) GetIssuer() string {
	if m.Server == nil {
		return ""
	}
	if m.Issuer == "" {
	...
	port := strings.Split(m.Addr(), ":")[2]
	return proto + "://" + m.Issuer + ":" + port + IssuerBase
}
...
func (m *MockOIDC) DiscoveryEndpoint() string {
	...
	return m.GetIssuer() + DiscoveryEndpoint
}
...
func (m *MockOIDC) AuthorizationEndpoint() string {
	...
	return m.GetIssuer() + AuthorizationEndpoint
}
...
func (m *MockOIDC) TokenEndpoint() string {
	...
	return m.GetIssuer() + TokenEndpoint
}
...
func (m *MockOIDC) UserinfoEndpoint() string {
	...
	return m.GetIssuer() + UserinfoEndpoint
}
...
func (m *MockOIDC) JWKSEndpoint() string {
	...
	return m.GetIssuer() + JWKSEndpoint
}
